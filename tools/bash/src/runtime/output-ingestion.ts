...
export class OutputIngestionController {
  ...
  accept(chunk: Buffer, stream: OutputStreamId = "output"): void {
    ...
    while (offset < chunk.length) {
      let slab = this.#lastSlab(stream);
      if (!slab || slab.end === slab.data.length) {
        slab = { kind: "data", stream, data: Buffer.allocUnsafe(this.#chunkBytes), start: 0, end: 0 };
  ...
  #drainTurn(): void {
    ...
    while (
      ...
      if (entry.kind === "end") {
        ...
        const accepted = Math.min(available, this.#turnBytes - processedBytes, this.#chunkBytes);
        const part = entry.data.subarray(entry.start, entry.start + accepted);
        try {
          this.#consume(part, entry.stream);
        } catch (error) {
          this.#onConsumeError?.(error);
        }
        entry.start += accepted;
  ...
  #lastSlab(stream: OutputStreamId): QueueSlab | undefined {
    if (!this.#hasQueuedEntries()) return undefined;
    const entry = this.#queue[this.#queue.length - 1];
    return entry?.kind === "data" && entry.stream === stream ? entry : undefined;
  }
