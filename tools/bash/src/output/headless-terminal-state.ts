...
export class HeadlessTerminalState {
  ...
  write(data: string, options: HeadlessTerminalWriteOptions = {}): void {
    ...
    if (!publishSnapshot) {
      ...
      if (this.#publicationBarrierGeneration === undefined) {
        this.#publicationBarrierGeneration = generation - 1;
      }
    }
    this.#pendingBytes += bytes;
    if (options.discardLastAlternateSnapshot) this.discardLastAlternateSnapshot();
    try {
  ...
  snapshot(): OutputSnapshot {
    this.#materializeSnapshot();
    const text = this.#snapshotActive
      ? this.#viewportSnapshot
      : this.#lastAlternatePending ? this.#lastAlternateSnapshot : "";
  ...
  snapshotDelta(): OutputSnapshot {
    ...
    return this.#snapshotText(this.#lastAlternatePending ? this.#lastAlternateSnapshot : "");
  }

  markDelivered(): void {
    if (this.#snapshotActive) this.#deliveredGeneration = this.#generation;
    else this.#lastAlternatePending = false;
  }
  ...
  #completeWrite(
    ...
    if (discardLastAlternateSnapshot && !this.#alternateActive) this.#lastAlternatePending = false;
    this.#resolveFlushWaiters();
    this.#notifyPressure();
  }
