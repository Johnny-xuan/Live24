...
export type TranscriptEvent =
  ...
  | { type: "string"; sourceChars: number; overflowed: boolean }
...
export class Ecma48Framer {
  ...
  #completeString(
    ...
    events.push({ type: "string", sourceChars, overflowed: this.#stringOverflowed });
    if (!this.#stringOverflowed) {
      terminal.push(`${this.#stringIntroducer}${this.#stringParts.join("")}${terminator}`);
    }
    this.#resetGround();
  }
