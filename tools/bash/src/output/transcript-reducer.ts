...
function isAltScreenBody(body: string): boolean {
  return body === "?1049" || body === "?1047" || body === "?47";
}
...
export class TranscriptReducer {
  ...
  #csi(event: Extract<TranscriptEvent, { type: "csi" }>): string {
    ...
    if (hasPrivateMarker && isAltScreenBody(body)) {
      if (final === "h") {
      ...
      if (final === "l" && this.#fullscreenActive) {
        this.#fullscreenActive = false;
    ...
    // DEC synchronized output is protocol-level evidence of a differential
    // screen. The reducer classifies its transaction boundary; Xterm remains
    // the sole owner of cells, cursor state, and the published viewport.
    if (hasPrivateMarker && body === "?2026") {
      if (final === "h") {
        const prefix = this.#enterScreenMode();
