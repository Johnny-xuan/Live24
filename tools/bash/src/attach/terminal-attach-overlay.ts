...
export class TerminalAttachOverlay implements Component, Focusable {
  ...
  render(width: number): string[] {
    ...
    const title = this.#theme.fg("accent", this.#theme.bold(` Bash #${this.#attachment.session_id} `));
    const meta = ` ${status} `;
    ...
    if (this.#menuOpen) {
      ...
      lines.push(row(this.#theme.fg("dim", ` ${this.#lastCols}×${this.#lastRows} • input is not logged by bash`)));
    }
