...
export function renderExecCall(args: Record<string, unknown>, theme: Theme): Text {
  const script = typeof args.command === "string" ? args.command : typeof args.cmd === "string" ? args.cmd : undefined;
  const nativeArgs = Array.isArray(args.args) ? args.args.filter((value): value is string => typeof value === "string") : [];
  const native = typeof args.executable === "string"
    ? [args.executable, ...nativeArgs.map((value) => /\s/.test(value) ? JSON.stringify(value) : value)].join(" ")
    : undefined;
  const command = script ?? native ?? "…";
  const prompt = native ? "»" : process.platform === "win32" ? "PS>" : "$";
  const workdir = typeof args.workdir === "string" ? ` @ ${args.workdir}` : "";
  const tty = args.tty === true ? " [tty]" : "";
  return new Text(theme.fg("toolTitle", theme.bold(`${prompt} ${command}${workdir}${tty}`)), 0, 0);
}
