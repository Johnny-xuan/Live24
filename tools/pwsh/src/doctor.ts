...
export interface DoctorReport {
  ...
  pty: { loaded: boolean; spawn_verified: boolean; error?: string };
...
export async function runDoctor(manager?: ProcessManager): Promise<DoctorReport> {
  const shell = resolveShell(process.platform === "win32" ? "$null" : "true", {
    login: process.platform !== "win32",
    preferPowerShell: process.platform === "win32",
    nativePowerShell: process.platform === "win32",
  });
  const ptyEnvironment = buildChildEnvironment(process.env, {}, { tty: true });
  const pty = await probePty({
    executable: shell.executable,
    args: shell.argv.slice(1),
    cwd: process.cwd(),
    env: ptyEnvironment,
  });
  ...
  const classification = classifyDoctor({
    ...
    artifactsEnabled,
    directoryMode,
    artifactSecurityIssue: artifacts.securityIssue,
  });
  const report: DoctorReport = {
    ...
    pty: { loaded: pty.loaded, spawn_verified: pty.spawnVerified, ...(pty.error ? { error: pty.error } : {}) },
    process_tree_strategy: process.platform === "win32" ? "taskkill /T /F" : "detached POSIX process group TERM→KILL",
    ctrl_c_strategy: process.platform === "win32" ? "PTY byte; pipes deterministic taskkill /T /F" : "PTY byte; pipes process-group SIGINT",
    artifacts: {
      enabled: artifactsEnabled,
      ...(manager && root ? { root } : {}),
      ...(directoryMode ? { directory_mode: directoryMode } : {}),
      ...(process.platform === "win32" && artifactsEnabled ? { directory_acl: "verified" as const } : {}),
...
export function formatDoctor(report: DoctorReport): string {
  return [
    `pwsh doctor: ${report.status}${report.issues.length ? ` (${report.issues.join(", ")})` : ""}`,
    ...
    `PTY: ${report.pty.loaded ? report.pty.spawn_verified ? "loaded (spawn verified)" : "loaded (import only)" : `unavailable (${report.pty.error ?? "unknown error"})`}`,
    `Process tree: ${report.process_tree_strategy}`,
    `Ctrl-C: ${report.ctrl_c_strategy}`,
    `Artifacts: ${report.artifacts.enabled ? "enabled" : "disabled"}${report.artifacts.root ? ` at ${report.artifacts.root}` : ""}${report.artifacts.directory_mode ? ` mode ${report.artifacts.directory_mode}` : ""}${report.artifacts.directory_acl ? " Windows DACL verified" : ""}`,
