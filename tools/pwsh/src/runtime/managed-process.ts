...
import { Ecma48Framer, terminalTextForEvent } from "../output/ecma48-framer.js";
...
export class ManagedProcess {
  ...
  async waitForInteraction(yieldMs: number, signal?: AbortSignal): Promise<number> {
    ...
    if (this.#exited && !this.#outputClosed) {
      ...
      if (remaining > 0) await this.#waitForState(this.#closeWaiters, () => this.#outputClosed, remaining, signal);
    }
    return Math.max(0, performance.now() - started);
  }

  write(data: string): void {
    if (!this.tty || !this.#pty) throw new Error(`stdin is closed for this session; rerun ${process.platform === "win32" ? "powershell" : "bash"} with tty=true to keep stdin open`);
    this.#pty.write(data);
  }
  ...
  async interrupt(): Promise<void> {
    ...
    if (process.platform === "win32") {
      // Windows pipe children have no shared interactive console to receive
      // GenerateConsoleCtrlEvent safely. Treat model Ctrl-C as deterministic
      // cancellation of the owned tree; PTYs still receive the literal byte.
      const terminated = await terminateWindowsProcessTree(this.#pid);
      if (!terminated) this.#failure ??= `Failed to terminate Windows process tree rooted at PID ${this.#pid}`;
      if (!this.#exited) await Promise.race([this.#waitForExit(), delay(1000)]);
      return;
    }
    try {
  ...
  async #terminateTree(): Promise<void> {
    if (process.platform === "win32") {
      ...
      if (this.#exited && !this.#outputClosed) {
        await this.#waitForState(this.#closeWaiters, () => this.#outputClosed, Math.max(POST_EXIT_CLOSE_GRACE_MS, 1000));
      }
  ...
  #projectFrame(frame: ReturnType<Ecma48Framer["write"]>, stream: OutputStreamId): void {
    for (const event of frame.events) {
      const wasFullscreenActive = this.#sanitizer.fullscreenActive;
      const committed = this.#sanitizer.consume([event]);
      if (stream === "pty" && this.#terminalState) {
        this.#terminalState.setSnapshotMode(this.#sanitizer.screenMode);
        if (!wasFullscreenActive && this.#sanitizer.fullscreenActive) {
          this.#terminalState.discardLastAlternateSnapshot();
        }
        const ordinaryOutputSupersedesAlternate = !this.#sanitizer.fullscreenActive
          && (
            committed.length > 0
            || this.#sanitizer.currentLineActive
            || (event.type === "text" && event.text.length > 0)
          );
        if (ordinaryOutputSupersedesAlternate) this.#terminalState.discardLastAlternateSnapshot();
        const terminal = terminalTextForEvent(event);
        if (terminal) {
          this.#terminalState.write(terminal, {
            publishSnapshot: !this.#sanitizer.synchronizedFrameActive,
            discardLastAlternateSnapshot: ordinaryOutputSupersedesAlternate,
          });
        }
      }
    }
