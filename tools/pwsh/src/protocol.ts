...
export const SuperShellParams = Type.Union([
  Type.Object(
    {
      command: Type.String({ minLength: 1, description: "PowerShell script on Windows; shell script on Unix." }),
      workdir: Type.Optional(Type.String({ minLength: 1, description: "Absolute workdir or path relative to the Pi session cwd." })),
      shell: Type.Optional(Type.String({ minLength: 1, description: "Explicit recognized script shell executable." })),
      login: Type.Optional(Type.Boolean({ description: "Load profile/login semantics. Defaults off on Windows and on elsewhere." })),
      tty: Type.Optional(Type.Boolean({ default: false, description: "Allocate a PTY/ConPTY and keep stdin writable." })),
      yield_time_ms: Type.Optional(Type.Integer({ minimum: 0, default: DEFAULT_EXEC_YIELD_TIME_MS, description: "Wait before yielding a live process; not a timeout." })),
      timeout: Type.Optional(Type.Number({ exclusiveMinimum: 0, maximum: MAX_TIMEOUT_SECONDS, description: "Hard process timeout in seconds; omitted means no timeout." })),
      max_output_tokens: Type.Optional(Type.Integer({ minimum: 0, maximum: OUTPUT_MAX_TOKENS, default: DEFAULT_MAX_OUTPUT_TOKENS })),
    },
  ...
  Type.Object(
    {
      executable: Type.String({ minLength: 1, description: "Native executable to spawn directly without shell parsing." }),
      args: Type.Optional(Type.Array(Type.String(), { default: [], description: "Exact argv entries." })),
      workdir: Type.Optional(Type.String({ minLength: 1, description: "Absolute workdir or path relative to the Pi session cwd." })),
      tty: Type.Optional(Type.Boolean({ default: false, description: "Allocate a PTY/ConPTY and keep stdin writable." })),
      yield_time_ms: Type.Optional(Type.Integer({ minimum: 0, default: DEFAULT_EXEC_YIELD_TIME_MS, description: "Wait before yielding a live process; not a timeout." })),
      timeout: Type.Optional(Type.Number({ exclusiveMinimum: 0, maximum: MAX_TIMEOUT_SECONDS, description: "Hard process timeout in seconds; omitted means no timeout." })),
...
export function resolveSuperShellInput(
  input: SuperShellInput,
  platform: NodeJS.Platform = process.platform,
):
  | { kind: "script"; execInput: ExecCommandInput; timeoutMs: number | undefined }
  | { kind: "native"; execInput: NativeExecInput; timeoutMs: number | undefined } {
  const raw = input as unknown as Record<string, unknown>;
  const hasCommand = typeof raw.command === "string";
  const hasExecutable = typeof raw.executable === "string";
  if (hasCommand === hasExecutable) throw new Error("provide exactly one of command or executable");
  if (input.timeout !== undefined && (!Number.isFinite(input.timeout) || input.timeout <= 0 || input.timeout > MAX_TIMEOUT_SECONDS)) {
    throw new Error(`timeout must be greater than 0 and at most ${MAX_TIMEOUT_SECONDS} seconds`);
  }
  const timeoutMs = input.timeout === undefined ? undefined : input.timeout * 1000;
  const common = {
  ...
  if (hasExecutable) {
    if (raw.shell !== undefined || raw.login !== undefined || raw.command !== undefined) {
      throw new Error("native executable mode does not accept command, shell, or login");
    }
    return {
      kind: "native",
      execInput: {
        executable: raw.executable as string,
  ...
  if (raw.args !== undefined || raw.executable !== undefined) {
    throw new Error("script mode does not accept executable or args");
  }
  return {
    kind: "script",
    execInput: {
      cmd: raw.command as string,
      ...(typeof raw.shell === "string" ? { shell: raw.shell } : {}),
      login: typeof raw.login === "boolean" ? raw.login : platform !== "win32",
      ...common,
    },
    timeoutMs,
  };
...
export function clampExecYield(
  ms: number,
  platform: NodeJS.Platform = process.platform,
  applyWindowsFloor = true,
): number {
  const floored = platform === "win32" && applyWindowsFloor ? Math.max(ms, WINDOWS_INITIAL_YIELD_FLOOR_MS) : ms;
